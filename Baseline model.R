...
Conc.h.cov2[2,]<-(1-TE.face.cov2*Frac.HF)*(Conc.h.cov2[1,]*exp(-Inact.h.cov2*Contact.time.face.w))
...
Conc.h.cov2.dataframe<-as.data.frame(t(Conc.h.cov2))
Conc.l.cov2.dataframe<-as.data.frame(t(Conc.l.cov2))
Dose.cov2.dataframe<-as.data.frame(t(Dose.cov2))
Risk.cov2.dataframe<-as.data.frame(t(Risk.cov2))

event<-rep(c(1:8),4000)
type<-c(rep("Hand",8000),rep("Laundry",8000),rep("Dose",8000),rep("Risk",8000))
value<-c(Conc.h.cov2.dataframe$`Hamper to washer`, Conc.h.cov2.dataframe$`Hand to face touch #1`,
         Conc.h.cov2.dataframe$`between face touch and end of washing cycle`,Conc.h.cov2.dataframe $`Washer to dryer`, 
         Conc.h.cov2.dataframe $`Hand to face touch #2`, Conc.h.cov2.dataframe $`Dry cycle`,
         Conc.h.cov2.dataframe $`Dryer to folding area`,Conc.h.cov2.dataframe $`Hand to face touch #3`,
         
         Conc.l.cov2.dataframe$`Hamper to washer`, Conc.l.cov2.dataframe$`Hand to face touch #1`,
         Conc.l.cov2.dataframe$`between face touch and end of washing cycle`,Conc.l.cov2.dataframe $`Washer to dryer`, 
         Conc.l.cov2.dataframe $`Hand to face touch #2`, 
         Conc.l.cov2.dataframe $`Dry cycle`,Conc.l.cov2.dataframe $`Dryer to folding area`,
         Conc.l.cov2.dataframe $`Hand to face touch #3`,
         
         Dose.cov2.dataframe$`Hamper to washer`, Dose.cov2.dataframe$`Hand to face touch #1`,
         Dose.cov2.dataframe$`between face touch and end of washing cycle`,Dose.cov2.dataframe $`Washer to dryer`,
         Dose.cov2.dataframe $`Hand to face touch #2`, Dose.cov2.dataframe $`Dry cycle`,Dose.cov2.dataframe $`Dryer to folding area`,
         Dose.cov2.dataframe $`Hand to face touch #3`,
         
         Risk.cov2.dataframe$`Hamper to washer`, Risk.cov2.dataframe$`Hand to face touch #1`,
         Risk.cov2.dataframe$`between face touch and end of washing cycle`,Risk.cov2.dataframe $`Washer to dryer`,
         Risk.cov2.dataframe $`Hand to face touch #2`, Risk.cov2.dataframe $`Dry cycle`,Risk.cov2.dataframe $`Dryer to folding area`,
         Risk.cov2.dataframe $`Hand to face touch #3`)

data<-data.frame(event,type,value)

ggplot(data)+geom_violin(aes(x=event,y=value,color=type, group=event))+
