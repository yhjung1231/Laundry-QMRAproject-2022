...
Risk.1[2,]<-1-exp(-2.46E-3*Dose.1[2,])
...
Risk.1[5,]<- 1-exp(-2.46E-3*Dose.1[5,])
...
Risk.1[8,]<- 1-exp(-2.46E-3*Dose.1[8,])
...
Risk.2[2,]<-1-exp(-2.46E-3*Dose.2[2,])
...
Risk.2[5,]<- 1-exp(-2.46E-3*Dose.2[5,])
...
Risk.2[8,]<- 1-exp(-2.46E-3*Dose.2[8,])
...
Risk.3[2,]<-1-exp(-2.46E-3*Dose.3[2,])
...
Risk.3[5,]<- 1-exp(-2.46E-3*Dose.3[5,])
...
Risk.3[8,]<- 1-exp(-2.46E-3*Dose.3[8,])
...
spear.SARS<-data.frame(T.handarea, Surface.area.laundry, Frac.HS, Frac.HF,Item.laundry, Dur.wash, Dur.dry,
                       Contact.time.laundry, Contact.time.face.w, Contact.time.face.d, Contact.time.face.f,
                       Reduc.wash, Reduc.dry, Reduc.hwash, TE.dry, TE.wet, TE.face,Freq.cough.day,Volume.cough, 
                       Conversion.ratio, Viralload, Conc.onecloth, Inact.h, Inact.s, Risk.3[8,])  

spear.anal<-cor(spear.SARS,method="spearman")
